...
export function crearCliente(cliente: string, arregloClientes: Array<Cliente>, arregloPacientes: Array<Paciente>):Array<Cliente> {
    ...
    let nuevoCliente: Cliente = new Cliente(idCliente,nombreCliente,telefonoCliente,clienteVip,cantidadVisitas,arregloPacientes);
    listaCliente.push(nuevoCliente);
    return listaCliente;
}
//funcion que genera un Objeto de tipo Proveedor desde una base de datos(.txt)==>
export function crearProveedor(proveedor: string, arregloProveedor: Array<Proveedor>): Array<Proveedor> {
    ...
    let nuevoProveedor :Proveedor = new Proveedor(idProveedor,nombreProveedor,telProveedor);
    listaProveedor.push(nuevoProveedor);
    return listaProveedor;
}
//funcion que genera un Objeto de tipo Paciente desde una base de datos(.txt)==>
export function crearPaciente(paciente: string, arregloPacientes: Array<Paciente>, arregloHistClinica: Array<string>):Array<Paciente> {
    ...
    let nuevoPaciente: Paciente = new Paciente(idPaciente,nombrePaciente,especie,arregloHistClinica);
...
//Cargamos los arreglos clientes, pacientes y proveedores--->
    ...
    let datosClientes : LectorArchivos = new LectorArchivos('./baseDeDatos/clientes.txt');
    let arregloClientes : Array<Cliente> = [];
    let datosPacientes : LectorArchivos = new LectorArchivos('./baseDeDatos/paciente.txt');
    let arregloPacientes : Array<Paciente> = [];
    let datosProveedor : LectorArchivos = new LectorArchivos('./baseDeDatos/proveedores.txt');
